...
const ADMIN_WA_NUMBER = "628567854448"; // Ganti dengan nomor WhatsApp asli Admin
...
export default function AssistantChat() {
  ...
  const handleSend = (textObj: string) => {
    ...
    // Cek apakah pertanyaan ada di daftar (case-insensitive)
    ...
    setTimeout(() => {
      ...
      if (foundQA) {
        ...
        // Di luar daftar → arahkan ke WhatsApp admin
        setMessages((prev) => [
          ...prev,
          {
            id: `bot_${Date.now()}`,
  ...
  return (
    <div
      className="pt-[4.21rem] min-h-screen flex flex-col"
      style={{ background: "linear-gradient(180deg, #F0F4F3 0%, #FAFBFC 100%)" }}
    >
      <TopBar withBackButton>Asisten AI AGPAII</TopBar>

      <div className="flex-1 overflow-y-auto p-4 md:px-8 lg:px-12 space-y-3 pb-32">
        {/* Messages Output */}
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
              ...
              >
                {/* Prevent html injection for user messages, but allow specifically generated bots HTML */}
                {m.isHtml && !isUser ? (
                  <div
                  ...
                  <div className="mt-4 flex flex-col gap-2">
                    {m.options.map((opt, i) => (
                      <button
                        ...
                        className={`text-left text-[13px] px-3 py-2.5 rounded-xl border font-medium transition-colors ${
                          opt.includes("Admin")
      ...
      <div className="fixed bottom-16 md:bottom-0 left-0 right-0 md:left-20 lg:left-64 z-50 bg-white/90 backdrop-blur-xl border-t border-slate-100 transition-all flex flex-col">
        {/* Input Bar */}
