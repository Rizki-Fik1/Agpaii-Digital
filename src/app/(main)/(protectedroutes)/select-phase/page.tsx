...
export default function SelectPhasePage() {
  ...
  useEffect(() => {
    if (jenjangId) {
      ...
      axios
        .get(
          `https://admin.agpaiidigital.org/api/master/fase?jenjang_id=${jenjangId}`,
        )
        .then((res) => {
      ...
      <div className="bg-[#009788] text-white px-6 py-8 rounded-b-[2rem] shadow-md relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-teal-100 text-sm leading-relaxed max-w-md">
            Pilih fase pembelajaran yang sesuai untuk melihat detail Capaian
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            [1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 animate-pulse"
              >
            ...
            <div className="text-center py-10 bg-white rounded-xl border border-dashed border-slate-300">
              <p className="text-slate-500">
                Tidak ada fase ditemukan untuk jenjang ini.
              </p>
            </div>
          )}
        </div>
