...
export default function DataForm() {
  ...
  const fetchDataOptions = async (operatorId: string, phone: string) => {
    ...
    try {
      const res = await fetch(
        `https://admin.agpaiidigital.org/api/filter-products?category_id=2`
      );
      const data = await res.json();
      if (!res.ok || !data.results) throw new Error("Gagal memuat paket data");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedCode) {
      await Swal.fire({
        ...
        text: "Pilih paket data terlebih dahulu",
        confirmButtonColor: "#009788",
      });
      return;
    }
    ...
    try {
      ...
      await Swal.fire({
        ...
        text: "Paket data sedang diproses dan akan aktif dalam beberapa saat.",
        confirmButtonColor: "#009788",
        width: "350px",
        padding: "1.5rem",
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-[480px] mx-auto px-6 sm:px-8 py-6">
        ...
        {/* Title Section */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-800">Beli Paket Data</h1>
          <p className="text-sm text-slate-500 mt-2">Pilih paket data sesuai kebutuhan Anda</p>
        </div>
          ...
          {selectedCode && (
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-[#009788]">
                  {formatRupiah(total)}
                </span>
              </div>
            </div>
