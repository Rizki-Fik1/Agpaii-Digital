...
export default function PLNBillCheckForm() {
  const [phone, setPhone] = useState("");
  const [noPelanggan, setNoPelanggan] = useState("");
  const [selectedProduct, setSelectedProduct] = useState(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [displayValue, setDisplayValue] = useState(""); // New state for input display
  const { auth } = useAuth();
  const [products, setProducts] = useState([]);
  const [originalProducts, setOriginalProducts] = useState([]); // Added missing state
  ...
  const fetchPLNProducts = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.products) {
        throw new Error(data.message || "Gagal mengambil data produk PLN");
      }
      ...
      setOriginalProducts(options);
      setProducts(options); // awal ditampilkan semua
    } catch (err) {
  ...
  const handleInputChange = (e) => {
    ...
    if (value.trim() === "") {
      ...
      setProducts([...startsWithMatches, ...includesMatches]);
    }
  };

  // Handle product selection
  const selectProduct = (productId) => {
    const selected = originalProducts.find((p) => p.id === productId);
  ...
  const handleInputFocus = () => {
    setIsDropdownOpen(true);
    if (searchTerm === "") {
      setProducts(originalProducts); // tampilkan semua kalau kosong
    }
  };

  const handleKeyDown = (e) => {
    if (!isDropdownOpen || products.length === 0) return;
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      selectProduct(products[highlightedIndex].id);
        ...
        <div className="mt-6 px-6 sm:px-8 border-t pt-4">
          ...
          <div className="mt-2 space-y-2 text-sm text-slate-700">
            ...
            <div className="flex justify-between">
              ...
              <span>Rp {billDetails.biaya_admin.toLocaleString("id-ID")}</span>
            </div>
            <div className="flex justify-between font-semibold">
              <span>Total Bayar:</span>
              <span>Rp {billDetails.jumlah_bayar.toLocaleString("id-ID")}</span>
            </div>
            <div className="flex justify-between">
