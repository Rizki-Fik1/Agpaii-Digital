...
export default function InternetBillCheckForm() {
  ...
  const fetchInternetProducts = async () => {
    ...
    try {
      const response = await fetch(
        "https://admin.agpaiidigital.org/api/tripay/postpaid/products?operator_id=101",
        {
          headers: {
  ...
  const handleCheckBill = async (e: React.FormEvent) => {
    ...
    if (!phone || !noPelanggan || !selectedProduct) {
      setError(
        "Harap isi nomor telepon, nomor pelanggan, dan pilih produk Internet!",
      );
      return;
    }
